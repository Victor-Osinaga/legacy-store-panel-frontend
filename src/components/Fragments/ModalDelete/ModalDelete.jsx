...
export default function ModalDelete({ id, closeModal, name, functionDelete, entity }) {
    const deleteCategoryById = <div>
        <p>¿Estás seguro que deseas eliminar esta categoria ? Los productos relacionados se moveran a una categoria <span className='text-danger text-uppercase fw-bold'>'uncategorized'</span></p>
        <p>Nombre: <strong>'{name}'</strong> - ID <strong>'{id}'</strong></p>
    </div>
    const deleteProductById = <div>
        <p>¿Estás seguro que deseas eliminar este producto ? Tambien se eliminaran los datos asociados a este</p>
        <p>Nombre: <strong>'{name}'</strong> - ID <strong>'{id}'</strong></p>
    </div>
