...
export default function RetiroSucursal() {
  ...
  const [allShipmentsLocalSelected, setAllShipmentsLocalSelected] =
    useState(false);
  const [showBtnActions, setShowBtnActions] = useState(false);
  const [modalInfo, setModalInfo] = useState({
    showModal: false,
    shipmentLocalId: null,
    shipmentLocalProvince: "",
  });
  ...
  const fetchShipmentsLocal = async () => {
    ...
    try {
      ...
      //     },
      // ]
      const shipmentsLocalWithSelection = shipmentsLocal.map((shipment) => ({
        ...shipment,
        selected: false,
      }));
      ...
      setTimeout(() => {
        ...
        if (shipmentsLocal.length == 0) {
          return toastSuccess(
            <>
              ...
              <strong>'{shipmentsLocal.length}'</strong>
            </>,
            id
          );
        }
        ...
        return toastSuccess(
          <>
            Sucursales cargadas: <strong>'{shipmentsLocal.length}'</strong>
          </>,
          id
        );
      }, 500);
  ...
  const selectShipmentLocal = (shipmentId) => {
    ...
    const algunaSeleccionada = newShipmentsLocal.some(
      (p) => p.selected == true
    );
    ...
    const ningunoSeleccionado = newShipmentsLocal.every(
      (shipment) => shipment.selected == false
    );
    const todosSeleccionados = newShipmentsLocal.every(
  ...
  const deleteShipmentsLocalSelected = () => {
    const shipmentsLocalSelected = shipmentsLocal.filter(
      (shipment) => shipment.selected
    );
    const idShipmentsLocalSelected = shipmentsLocalSelected.map(
      (prod) => prod.id
    );
  ...
  const eliminarShipmentLocalPorId = async (id, shipmentProvice) => {
    ...
    try {
      ...
      setShipmentsLocal(shipmentsLocalWithSelection);
      console.log(
        "nuevas sucursales desde deleteById",
        shipmentsLocalWithSelection
      );
      return toastSuccess(
        <>
          Sucursal eliminada: <strong>'{shipmentProvice}'</strong> - cantidad:{" "}
  ...
  return (
    <>
      <section className="containerViewMain">
        <MainTitle
          mainTitle="Retiro en sucursal"
          linkToCreate="/admin/retiro-en-sucursal/crear"
          titleButton="Nueva sucursal"
        />
        <div className="text-secondary mb-2 fontSM-Custom">
          <p className="m-0">
            Aqui estara tu sucursal para que tus clientes puedan retirar los
          ...
          shipmentsLocal.length > 0 ? (
            <>
              <ActionsInGroup
                ...
                deleteGroup={deleteShipmentsLocalSelected}
              />
                  ...
                  <tbody className="fontSM-Custom">
                    {shipmentsLocal.map((shipment, index) => (
                      <tr key={shipment.id}>
                        ...
                        <td className="fontSM-Custom textGray500-Custom tdSpacingMain">
                          <span title={shipment.streetName}>
                            {truncarTexto(shipment.streetName)}
                          </span>
                        </td>
                        <td className="fontSM-Custom textGray500-Custom tdSpacingMain">
