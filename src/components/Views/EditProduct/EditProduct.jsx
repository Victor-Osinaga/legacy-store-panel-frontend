...
export default function EditProduct() {
    ...
    const { register, handleSubmit, formState: { errors }, watch, reset, getValues, control, setValue } = useForm({
        defaultValues: {
            ...
            categories: [
                {
                    categoria: { id: '', name: '' },
                    subCategoria: { id: '', name: '' },
                    subSubCategoria: { id: '', name: '' },
                }
            ],
            description: "",
            price: "",
    ...
    const { fields: categoryFiels, append: appendCategory, remove: removeCategory } = useFieldArray({
        control,
        name: "categories"
    });
    ...
    const handlePrimaryCategoryChange = (e) => {
        ...
        const selectedCategoryIndex = categories.findIndex(c => c.categoria.id == selectedPrimaryCategoryId);

        if (selectedCategoryIndex !== -1) {
            const selectedCategory = categories[selectedCategoryIndex];
            console.log("selectedCategory", selectedCategory);

            // Actualiza los valores en React Hook Form
            setValue(`categories.${selectedCategoryIndex}.categoria.id`, selectedCategory.id);
            setValue(`categories.${selectedCategoryIndex}.categoria.name`, selectedCategory.name);

            // Restablece los valores de las categorías secundarias y terciarias
            reset({
                ...getValues(),
                [`categories.${selectedCategoryIndex}.categoria.id`]: selectedCategory.id,
                [`categories.${selectedCategoryIndex}.categoria.name`]: selectedCategory.name,
                // secondaryCategory: '',
                // terciaryCategory: ''
            });
        }
    ...
    const onSubmit = async (data) => {
        ...
        for (const key in data) {
            ...
            if (key == 'sizes') {
                ...
                formData.append(key, JSON.stringify(data.categories));
            }else {
        ...
        const toastId = toastLoading("Editando producto...")

        try {
            console.log("final", formData);
            const editedProduct = await editProductById(productId,formData)
            toastSuccess(<>Producto editado: <strong>'{editedProduct.name}'</strong>, cantidad: <strong>'1'</strong></>, toastId)

            // Resetear el formulario si es necesario
            return reset(editedProduct);
        } catch (error) {
            toastError(
        ...
        <>
            <section className='containerViewMain'>
                <div className='d-flex justify-content-between align-items-center w-100'>
                    <div>
                        <h3 className='fs-4 mainTitle'>Editar Producto</h3>
                    </div>
                </div>
                                ...
                                <div className='row mb-3'>
                                    ...
                                    <div className='row rounded border col-12 p-0 py-2 mx-auto'>
                                        <div className='col d-flex flex-column mb-2'>
                                            ...
                                            {categories ? (
                                                categories.length > 0 ? (
                                                    categoryFiels.map((cat, categoryIndex) => (
                                                        <div key={cat.id}>
                                                            <select
                                                                // defaultValue={cat.categoria.id}
                                                                className="form-select fontSM-Custom" aria-label="Default select example" {
                                                                ...register(`categories.${categoryIndex}.categoria.id`, {
                                                                    required: {
                                                                        value: true,
                                                                        message: 'Requerido'
                                                                    },
                                                                    validate: (value) => {
                                                                        if (!categories.find(c => c.id == value)) {
                                                                            ...
                                                                            const categorieFind = categories.find(c => c.id == value)
                                                                            // console.log("name in obj", getValues(`categories.${categoryIndex}.categoria.name`));
                                                                            setValue(`categories.${categoryIndex}.categoria.name`, categorieFind.name);
                                                                        }
                                                                    }
                                                                })
                                                                }>
                                                            ...
                                                            {errors.categories?.[categoryIndex]?.categoria?.id && <span className='mt-1 fontXS-Custom text-danger'>{errors.categories?.[categoryIndex]?.categoria.id?.message} <span className='fw-semibold'>*</span></span>}
                                            ...
                                            {categories ? (
                                                categories.length > 0 ? (
                                                    categoryFiels.map((cat, categoryIndex) => (
                                                        <div key={cat.id}>
                                                            <select
                                                                // defaultValue={cat.subCategoria.id}
                                                                onChange={handlePrimaryCategoryChange} // Tu lógica personalizada
                                                                className="form-select fontSM-Custom" aria-label="Default select example" {
                                                                ...register(`categories.${categoryIndex}.subCategoria.id`, {
                                                                    required: {
                                                                        value: true,
                                                                        message: 'Requerido sub'
                                                                    },
                                                                    validate: (value) => {
                                                                        const primaryId = getValues(`categories.${categoryIndex}.categoria.id`)
                                                                        const categorie = categories.find(c => c.id == primaryId)
                                                                        const sub = categorie.subCategories.find(sub => sub.id == value)
                                                                        if (!sub) {
                                                                            return "Opcion invalida-sub"
                                                                        }
                                                                        else {
                                                                            setValue(`categories.${categoryIndex}.subCategoria.name`, sub.name)
                                                                        }
                                                                    }
                                                                })
                                                                }>
                                                                ...
                                                                {
                                                                    categories.map((cat, index) => {
                                                                        if (cat.id == getValues(`categories.${categoryIndex}.categoria.id`)) {
                                                            ...
                                                            {/* siempre se condciona el mensaje de error hacia el campo especifico del error  */}
                                                            {errors.categories?.[categoryIndex]?.subCategoria?.id && <span className='mt-1 fontXS-Custom text-danger'>{errors.categories?.[categoryIndex]?.subCategoria.id?.message} <span className='fw-semibold'>*</span></span>}
                                                            {/* {errors.primaryCategory && <span className='mt-1 fontXS-Custom text-danger'>{errors.primaryCategory.message} <span className='fw-semibold'>*</span></span>} */}
                                                        </div>
                                                    ))
                                                    // <>
                                                        ...
                                                        categoryFiels.map((cat, categoryIndex) => (
                                                            <div key={cat.id}>
                                                                <select
                                                                    ...
                                                                    onChange={handlePrimaryCategoryChange} // Tu lógica personalizada
                                                                    className="form-select fontSM-Custom" aria-label="Default select example" {
                                                                    ...register(`categories.${categoryIndex}.subSubCategoria.id`, {
                                                                        required: {
                                                                            value: true,
                                                                            message: 'Requerido subsub'
                                                                        },
                                                                        validate: (value) => {
                                                                            const primaryId = getValues(`categories.${categoryIndex}.categoria.id`)
                                                                            const secondaryId = getValues(`categories.${categoryIndex}.subCategoria.id`)
                                                                            const categorie = categories.find(c => c.id == primaryId)
                                                                            const sub = categorie.subCategories.find(sub => sub.id == secondaryId)
                                                                            const subsub = sub.categories.find(c=> c.id == value)
                                                                            console.log("subsub",subsub);
                                                                            if(!subsub){
                                                                                console.log("error en validate terciary");
                                                                                return "Opcion invalida-subsub"
                                                                            }
                                                                             else {
                                                                                    setValue(`categories.${categoryIndex}.subSubCategoria.name`, subsub.name);
                                                                                }
                                                                        }
                                                                    })
                                                                    }>
                                                                    <option value="">Opciones</option>
                                                                    {
                                                                        categories.map((cat, index) => {
                                                                            if (cat.id == getValues(`categories.${categoryIndex}.categoria.id`)) {
                                                                                return cat.subCategories.map((sub, i) => {
                                                                                    if (sub.id === getValues(`categories.${categoryIndex}.subCategoria.id`)) {
                                                                                        return sub.categories.map(sub2 => (
                                                                                            <option key={sub2.id} value={sub2.id}>{sub2.name}</option>
                                                                                        ))
                                                                                    }
                                                                                })
                                                                            }
                                                                ...
                                                                {/* siempre se condciona el mensaje de error hacia el campo especifico del error  */}
                                                                {errors.categories?.[categoryIndex]?.subSubCategoria?.id && <span className='mt-1 fontXS-Custom text-danger'>{errors.categories?.[categoryIndex]?.subSubCategoria?.id.message} <span className='fw-semibold'>*</span></span>}
                                                                {/* {errors.primaryCategory && <span className='mt-1 fontXS-Custom text-danger'>{errors.primaryCategory.message} <span className='fw-semibold'>*</span></span>} */}
                                                            </div>
                                                        ))
                                                        // <>
