...
export default function Categories() {
    const { selectAll, toastLoading, toastSuccess, toastError, dismissToast, showItemActions, truncarTexto } = useStoreContext()
    ...
    const [modalInfo, setModalInfo] = useState({ showModal: false, categorieId: null, categorieName: '' });
    ...
    const fetchCategories = async () => {
        ...
        try {
            ...
            const categoriasWithSelection = categorias.map(cat => ({ ...cat, selected: false }));
    ...
    const eliminarCategoriaPorId = async (categorieId, categorieName) => {
        ...
        try {
            ...
            console.log("nuevas categorias desde deleteById", categoriesWithSelection);
                                ...
                                <table className='tableContainerViewMain bg-white w-100'>
                                    <thead className='tableHeadTitlesMain fontXS-Custom textGray-Custom'>
                                        <tr>
                                            ...
                                            <th className='thSpacingMain'><span>PRIMARIA</span></th>
                                            <th className='thSpacingMain'><span>SECUNDARIA</span></th>
                                            <th className='thSpacingMain'><span>TERCIARIA</span></th>
                                            <th className='thSpacingMain'><span>PRODUCTOS</span></th>
                                        ...
                                        {categories.map((cat, index) => (
                                            ...
                                            <tr key={cat.id}>
                                                ...
                                                <td className="fontSM-Custom tdSpacingMain">
                                                    <Link className="trBodyLinkNameMain" to={`/admin/categorias/editar/${cat.id}`}>
                                                        {/* <img className='productImgMain' src={cat.image} alt={cat.name} /> */}
                                                        <span title={cat.name}>{truncarTexto(cat.name)}</span>
                                                    </Link>
                                                </td>
                                                <td className="fontSM-Custom tdSpacingMain"><span>{cat.subCategories.length} (total)</span></td>
                                                <td className="fontSM-Custom textGray-Custom tdSpacingMain fw-bold"><span>{calculateTerciaryCategories(cat.subCategories)} (total)</span></td>
                                                <td className="fontSM-Custom textGray-Custom tdSpacingMain fw-bold"><span>{getTotalProductsByCategoryId(cat.id, products)} (total)</span></td>
                                                ...
                                                <td className='fontSM-Custom tdSpacingMain position-relative'>
                                                    ...
                                                    <div className='d-flex '>
                                                        ...
                                                        <button onClick={() => showItemActions(cat.id)} className="btnMoreActionsMain /*w-100*/ position-relative border border-0">
                                                            <OutlineMore />
                                                        </button>
