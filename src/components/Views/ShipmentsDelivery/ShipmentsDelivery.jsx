...
export default function ShipmentsDelivery() {
    ...
    const [allShipmentsDeliverySelected, setAllShipmentsDeliverySelected] = useState(false);
    const [showBtnActions, setShowBtnActions] = useState(false);
    const [modalInfo, setModalInfo] = useState({ showModal: false, shipmentDeliveryId: null, shipmentDeliveryProvince: '' });
        ...
        try {
            ...
            setTimeout(() => {
                ...
                if (shipmentsDelivery.length == 0) {
                    return toastSuccess(<>No hay sucursales cargadas: <strong>'{shipmentsDelivery.length}'</strong></>, id)
                }

                return toastSuccess(<>Provincias cargadas: <strong>'{shipmentsDelivery.length}'</strong></>, id)
            }, 500)
        } catch (error) {
            toastError(
    ...
    const selectShipmentDelivery = (shipmentId) => {
        ...
        const algunaSeleccionada = newShipmentsDelivery.some(p => p.selected == true)
        if (algunaSeleccionada) {
            setAllShipmentsDeliverySelected(false)
            setShowBtnActions(true)
        }

        const ningunoSeleccionado = newShipmentsDelivery.every(shipment => shipment.selected == false)
        const todosSeleccionados = newShipmentsDelivery.every(shipment => shipment.selected == true)
    ...
    const deleteShipmentsDeliverySelected = () => {
        const shipmentsDeliverySelected = shipmentsDelivery.filter(shipment => shipment.selected)
        const idShipmentsDeliverySelected = shipmentsDeliverySelected.map(prod => prod.id)
    ...
    const openModal = (id, name) => {
        showItemActions(id)
        setModalInfo({ showModal: true, shipmentDeliveryId: id, shipmentDeliveryProvince: name });
    };
    ...
    return(
        <>
            <section className="containerViewMain">
                <MainTitle mainTitle="Envios a provincias" linkToCreate="/admin/envios/crear" titleButton="Crear provincia"/>
                <div className="text-secondary mb-2 fontSM-Custom">
                    <p className="m-0">Aqui estaran todas las provincias a las que admitas enviar tus productos con su costo</p>
                </div>

                {!loading ? (
                    shipmentsDelivery.length > 0 ? (
                        <>
                            <ActionsInGroup handlebtnActionMains={showBtnActions} deleteGroup={deleteShipmentsDeliverySelected}/>
                                            ...
                                            <tr key={shipment.id}>
                                                ...
                                                <td className="fontSM-Custom tdSpacingMain position-relative">
                                                    <div className="d-flex">
                                                        ...
                                                        <ul id={shipment.id} className={`submenuPruebaMain shadow-lg ${index === shipmentsDelivery.length - 1 && index !== 0 ? '' : ''}`}>
                                                            <li className="btnsMoreActionsContainerMain">
                                                                <Link to={`/admin/envios/editar/${shipment.id}`}>
