...
export default function OrderDetail() {
  ...
  const fetchOrder = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        toastSuccess(
          <>
            Orden cargada: <strong>'{order.id}'</strong>
          </>,
          id
        );
      }, 500);
      ...
      toastError(
        <div className="text-center">
          <p className="mb-0">
            <strong>{error.msg || "Error al cargar la orden"}</strong>
          </p>
        </div>,
        id
      );
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      toastSuccess(
        <>
          Orden actualizada: <strong>'{order.id}'</strong>
        </>,
        toastId
      );
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="d-flex justify-content-between align-items-center w-100 mb-2">
            ...
            <button className="btnNewContainer rounded py-1 border-0">
              <div className="d-flex px-4 justify-content-between align-items-center">
                ...
                <div
                  type="submit"
                  className="btnNew border border-0 text-white bg-transparent"
                >
                  Actualizar orden
                </div>
              </div>
          ...
          <div className="mb-3">
            <label
              ...
              className="mb-1 fw-bold textGray500-Custom"
            >
              Actualiza el estado:
            </label>
            <select
            ...
            <>
              <div className="bg-white py-3 px-4 rounded-3 shadow-lg mb-3">
                <h4 className="m-0 fw-bold mb-3">
                  <svg
                    className="textGray500-Custom svgSize"
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
                ...
                <p>
                  <span className="fw-bold">ID de orden:</span> {order.id}
                </p>
                <p>
                  <span className="fw-bold">ID de pago:</span>{" "}
                  {order.payment_id}
                </p>
                <p>
                  <span className="fw-bold">Estado:</span>{" "}
                  <span
                    className="text-capitalize status_order"
                ...
                <p>
                  <span className="fw-bold">Fecha:</span> {order.timestamp}
                </p>
                <p>
                  <span className="fw-bold">Cuotas:</span> {order.installments}
                </p>
              </div>
              ...
              <div className="bg-white py-3 px-4 rounded-3 shadow-lg mb-3">
                ...
                <p>
                  ...
                  {order.client_info_contact.name}
                </p>
                <p>
                  <span className="fw-bold">Apellido:</span>{" "}
                  {order.client_info_contact.surname}
                </p>
                <p>
                  <span className="fw-bold">Email:</span>{" "}
                  {order.client_info_contact.email}
                </p>
                <p>
                  <span className="fw-bold">Telefono:</span> (
                  {order.client_info_contact.phone.area_code}){" "}
                  {order.client_info_contact.phone.number}
                </p>
              </div>
              ...
              <div className="bg-white py-3 px-4 rounded-3 shadow-lg mb-3">
                ...
                <p>
                  ...
                  {order.shipment_info.street_name} -{" "}
                  {order.shipment_info.street_number}
                </p>
                <p>
                  <span className="fw-bold">Provincia:</span>{" "}
                  {order.shipment_info.province}
                </p>
                <p>
                  <span className="fw-bold">Localidad:</span>{" "}
                  {order.shipment_info.locality}
                </p>
                <p>
                  <span className="fw-bold">Código postal:</span>{" "}
                  {order.shipment_info.postal_code}
                </p>
                <p>
                  <span className="fw-bold">Tipo de envio:</span>{" "}
                  {order.shipment_info.shipment_type}
                </p>
              </div>
                    ...
                    >
                      ...
                      <div className="d-flex flex-column justify-content-center align-items-start">
                        ...
                        >
                          <p className="m-0 d-flex align-items-center">
                            Talla: {item.selected_size_name}, Color:{" "}
                      ...
                      <div className="d-flex flex-column justify-content-center align-items-end">
                        ...
                        <p className="text-success fw-bold">
                          ${toNumberArgStandard(item.quantity * item.price)}
                        </p>
                      </div>
                    </div>
                  </>
              ...
              <div className="bg-white py-3 px-4 rounded-3 shadow-lg mb-3">
                ...
                <p>
                  <span className="fw-bold">Subtotal: </span>$
                  {toNumberArgStandard(calcPriceFinal(order.products, 0))}
                </p>
                <p>
                  <span className="fw-bold">Costo de retiro: </span>$
                  {toNumberArgStandard(order.shipment_info.shipment_cost)}
                </p>
                <p>
                  <span className="fw-bold">Total pagado: </span>$
                  {toNumberArgStandard(order.total_paid_amount)}
                </p>
                <p>
                  <span className="fw-bold">Monto neto recibido: </span>
                  <span className="text-success fw-bold">
                    ${toNumberArgStandard(order.net_received_amount)}
                  </span>
                </p>
                <p>
                  <span className="fw-bold">Fecha de liberación: </span>
